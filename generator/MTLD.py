...
def data_wash2():
    with open('patent2', 'r', encoding='utf-8') as f:
        with open('patent5', 'w', encoding='utf-8') as w:
            mean_MTLD = []
            lines = f.readlines()
            i = 0
            print(len(lines))
...
def data_wash_7():
    with open('patent4','r',encoding='utf-8') as f:
        with open('patent5','w',encoding='utf-8') as w:
            lines = f.readlines()
            lines = set(lines)
            i = 0
...
def data_wash_8():
    with open('patent5', 'r', encoding='utf-8') as f:
        lines = f.readlines()
        mean_MTLD = []
        i = 0
        with open('patent6', 'w', encoding='utf-8') as w:
            for line in lines:
                line = line.replace('\n', '')
                if len(list(line)) < 50:
                    continue
                MTLD_line = mtld2(list(line))
                if MTLD_line > 40:
                    line_split = line.split(' && ')
