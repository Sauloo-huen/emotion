...
def train():
    ...
    for epoch in range(config.num_epochs):
        ...
        for i, batch in enumerate(train_iter): # token_ids, label, seq_len, mask
            ...
            predicted_token_id = predicted_token_id.cpu().numpy()
            ...
            if i % 100 == 0:
                ...
                labels_all = labels_all.reshape([int(len(labels_all) / 2), 2])
                predict_all = predict_all.reshape([int(len(predict_all) / 2), 2])
                acc = 0
